...
import { jsonToJava } from '@/lib/json-to-java';
import { 
  Braces, 
  Copy, 
  Trash2, 
...
const highlightJava = (code: string) => {
  ...
  // Basic Regex-based highlighting for Java
  const tokens = code.split(/(\b(?:public|private|protected|class|static|void|interface|extends|implements|package|import|return|new|if|else|for|while|try|catch|String|Integer|Double|Boolean|List|Object|int|double|boolean)\b|@\w+|".*?"|\/\/.*)/g);
  
  return tokens.map((token, i) => {
    if (!token) return null;
    
    // Keywords
    if (/^(public|private|protected|class|static|void|interface|extends|implements|package|import|return|new|if|else|for|while|try|catch)$/.test(token)) {
      return <span key={i} className="text-purple-600 dark:text-purple-400 font-semibold">{token}</span>;
    }
    // Types
    if (/^(String|Integer|Double|Boolean|List|Object|int|double|boolean)$/.test(token)) {
      return <span key={i} className="text-blue-600 dark:text-blue-400">{token}</span>;
    }
    // Annotations
    if (token.startsWith('@')) {
      return <span key={i} className="text-orange-600 dark:text-orange-400 italic">{token}</span>;
    }
    // Strings
    if (token.startsWith('"')) {
...
export default function JsonToJavaPage() {
  ...
  const [packageName, setPackageName] = useState('com.example.generated');
  ...
  const handleConvert = () => {
    ...
    const result = jsonToJava(input, { rootName, packageName, useJackson, useLombok });
  ...
  const highlightedOutput = useMemo(() => highlightJava(output), [output]);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 py-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h1 className="text-3xl font-headline font-bold">{t('tools.json_to_java.name')}</h1>
          <p className="text-muted-foreground">{t('tools.json_to_java.description')}</p>
        </div>
      </div>
        ...
        <div className="flex flex-row lg:flex-col justify-center items-center gap-4 py-4 lg:py-0 min-w-[240px]">
          <div className="w-full space-y-4 px-2">
            ...
            <div className="flex items-center justify-between px-1">
              <Label htmlFor="use-jackson" className="text-xs font-medium cursor-pointer">
                Jackson Annotations
              </Label>
              <Switch 
                id="use-jackson" 
                checked={useJackson} 
                onCheckedChange={setUseJackson} 
              />
            </div>

            <div className="flex items-center justify-between px-1">
              <Label htmlFor="use-lombok" className="text-xs font-medium cursor-pointer">
                Lombok Annotations
              </Label>
              <Switch 
                id="use-lombok" 
                checked={useLombok} 
                onCheckedChange={setUseLombok} 
              />
            </div>
          </div>
          ...
          <CardContent className="flex-1 flex flex-col p-0 overflow-hidden">
            <div className="flex flex-1 min-h-[450px] border-t bg-secondary/10 overflow-hidden">
              ...
              <div className="flex-1 overflow-auto bg-[#fafafa] dark:bg-[#0f1115]">
                <div className="font-code w-full min-h-full leading-6 py-2.5 px-3 whitespace-pre overflow-x-auto text-xs">
                  {highlightedOutput || <span className="text-muted-foreground/50">Java class will appear here...</span>}
                </div>
              </div>
            </div>
            {error && (
