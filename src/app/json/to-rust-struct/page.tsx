...
import { jsonToRust } from '@/lib/json-to-rust';
import { 
  Braces, 
  Copy, 
  Trash2, 
...
const highlightRust = (code: string) => {
  ...
  // Basic Regex-based highlighting for Rust
  const tokens = code.split(/(\b(?:struct|pub|type|use|fn|let|mut|match|if|else|impl|trait|for|in|while|loop|return|as|String|i64|f64|bool|Vec|Option|serde_json|Value)\b|#\[.*?\]|\/\/.*)/g);
  
  return tokens.map((token, i) => {
    if (!token) return null;
    
    // Keywords
    if (/^(struct|pub|type|use|fn|let|mut|match|if|else|impl|trait|for|in|while|loop|return|as)$/.test(token)) {
...
export default function JsonToRustPage() {
  ...
  const [derives, setDerives] = useState('Default, Debug, Serialize, Deserialize');
  const [error, setError] = useState<string | null>(null);

  const handleConvert = () => {
    if (!input.trim()) return;
    const result = jsonToRust(input, { rootName, isPublic, derives });
  ...
  const highlightedOutput = useMemo(() => highlightRust(output), [output]);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 py-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h1 className="text-3xl font-headline font-bold">{t('tools.json_to_rust.name')}</h1>
          <p className="text-muted-foreground">{t('tools.json_to_rust.description')}</p>
        </div>
      </div>
        ...
        <div className="flex flex-row lg:flex-col justify-center items-center gap-4 py-4 lg:py-0 min-w-[240px]">
          <div className="w-full space-y-4 px-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                value={derives}
                onChange={(e) => setDerives(e.target.value)}
                className="h-9 bg-background text-xs"
              />
            </div>

            <div className="flex items-center justify-between px-1">
              <Label htmlFor="public-structs" className="text-xs font-medium cursor-pointer">
                Public Structs (pub)
              </Label>
              <Switch 
                id="public-structs" 
            ...
            <div className="flex flex-1 min-h-[450px] border-t bg-secondary/10 overflow-hidden">
              ...
              <div className="flex-1 overflow-auto bg-[#fafafa] dark:bg-[#0f1115]">
                <div className="font-code w-full min-h-full leading-6 py-2.5 px-3 whitespace-pre overflow-x-auto text-xs">
                  {highlightedOutput || <span className="text-muted-foreground/50">Rust definitions will appear here...</span>}
                </div>
              </div>
            </div>
            {error && (
