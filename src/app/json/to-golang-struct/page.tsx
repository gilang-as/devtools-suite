...
const highlightGo = (code: string) => {
  ...
  // Basic Regex-based highlighting for Go
  const tokens = code.split(/(\b(?:type|struct|package|import|func|var|const|interface|string|int|float64|bool|interface\{\})\b|`json:"[^"]+"`|\/\/.*)/g);
  
  return tokens.map((token, i) => {
    if (!token) return null;
    
    // Keywords
    if (/^(type|struct|package|import|func|var|const|interface)$/.test(token)) {
      return <span key={i} className="text-purple-600 dark:text-purple-400 font-semibold">{token}</span>;
    }
    // Types
    if (/^(string|int|float64|bool|interface\{\})$/.test(token)) {
      return <span key={i} className="text-blue-600 dark:text-blue-400">{token}</span>;
    }
    // JSON Tags
    if (token.startsWith('`')) {
      return <span key={i} className="text-green-600 dark:text-green-400 italic">{token}</span>;
    }
    // Comments
    if (token.startsWith('//')) {
...
export default function JsonToGolangPage() {
  ...
  const highlightedOutput = useMemo(() => highlightGo(output), [output]);
            ...
            <div className="flex flex-1 min-h-[450px] border-t bg-secondary/10 overflow-hidden">
              ...
              <div className="flex-1 overflow-auto bg-[#fafafa] dark:bg-[#0f1115]">
                <div className="font-code w-full min-h-full leading-6 py-2.5 px-3 whitespace-pre overflow-x-auto">
                  {highlightedOutput || <span className="text-muted-foreground/50">Go struct will appear here...</span>}
                </div>
              </div>
            </div>
            {error && (
