...
const highlightJs = (code: string) => {
  ...
  // VS Code style regex-based highlighting
  const tokens = code.split(/(\b(?:const|let|var|function|return|if|else|for|while|import|export|from|class|extends|new|try|catch|finally|async|await|type|interface|enum|static|private|public|protected|readonly|as|any|string|number|boolean|void|null|undefined|true|false)\b|"(?:\\.|[^\\"])*"|'(?:\\.|[^\\'])*'|`(?:\\.|[^\\`])*`|\/\/.*|\/\*[\s\S]*?\*\/|[-+*/%=<>!&|^~]+|\d+)/g);
  
  return tokens.map((token, i) => {
    if (!token) return null;
    
    // Keywords
    if (/^(const|let|var|function|return|if|else|for|while|import|export|from|class|extends|new|try|catch|finally|async|await|type|interface|enum|static|private|public|protected|readonly|as|any|string|number|boolean|void|null|undefined)$/.test(token)) {
...
export default function JavascriptFormatterPage() {
  ...
  const handleBeautify = () => {
    ...
    setOutput(beautifyJs(input, indentSize));
  };
  ...
  const highlightedOutput = useMemo(() => highlightJs(output), [output]);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 py-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h1 className="text-3xl font-headline font-bold">{t('tools.js_formatter.name')}</h1>
          <p className="text-muted-foreground">{t('tools.js_formatter.description')}</p>
        </div>
      </div>
        ...
        <Card className="flex-1 border-border shadow-lg flex flex-col overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            ...
            <div className="flex gap-2">
              <input type="file" accept=".js" className="hidden" ref={fileInputRef} onChange={handleFileUpload} />
        ...
        <div className="flex flex-row lg:flex-col justify-center items-center gap-4 py-4 lg:py-0 min-w-[200px]">
          <div className="w-full space-y-4 px-2">
            <div className="space-y-1.5">
              ...
              <Select value={indentSize.toString()} onValueChange={(val) => setIndentSize(parseInt(val))}>
            ...
            <div className="flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="word-wrap" className="text-xs font-medium cursor-pointer">{t('common.wordWrap')}</Label>
              </div>
              <Switch id="word-wrap" checked={wordWrap} onCheckedChange={setWordWrap} />
            </div>
          </div>
          <div className="flex flex-col gap-3 w-full">
            <Button onClick={handleBeautify} className="shadow-md bg-primary hover:bg-primary/90 w-full" size="lg">
            ...
            <Button onClick={handleMinify} variant="secondary" className="w-full shadow-sm" size="lg">
