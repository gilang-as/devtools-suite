...
export default function YamlToJsonPage() {
  ...
  const handleConvert = () => {
    ...
    try {
      const result = yamlToJson(input, indentSize);
      setOutput(result);
      setError(null);
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 py-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h1 className="text-3xl font-headline font-bold">{t('tools.yaml_to_json.name')}</h1>
          <p className="text-muted-foreground">{t('tools.yaml_to_json.description')}</p>
        </div>
      </div>
