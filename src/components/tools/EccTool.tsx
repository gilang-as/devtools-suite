...
import { Zap, Key, FileText, Copy, CheckCircle2, XCircle, Activity } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { cn } from '@/lib/utils';

interface EccToolProps {
  type: 'sign' | 'verify';
}

export default function EccTool({ type }: EccToolProps) {
  ...
  const navItems = [
    { name: 'Sign', href: '/security/ecc/sign' },
    { name: 'Verify', href: '/security/ecc/verify' },
  ];
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 py-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h1 className="text-3xl font-headline font-bold">ECC Identity Tool</h1>
          <p className="text-muted-foreground">{t(`tools.ecc_${type}.description`)}</p>
        </div>
      </div>
          ...
          <Card className="border-border shadow-lg">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2">
                ...
                {type === 'sign' ? 'Private Key (PKCS#8)' : 'Public Key (SPKI)'}
              </CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={handleAction} className="w-full h-11" size="lg">
                ...
                {type === 'sign' ? 'Generate Signature' : 'Verify Identity'}
              </Button>
            </CardContent>
          </Card>
        </div>
